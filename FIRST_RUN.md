...
当前状态（2026-09-24，依据学校终端输出）：已修复官方旧辅助头兼容问题；318 帧完整真实扫查在学校 CUDA 上严格加载权重并成功保存三个结果文件。下一步是[检查结果与三维预览](docs/FIRST_PREVIEW.md)，几何质量仍待查看。
...
- `requirements-inference.txt`：首轮推理依赖；学校已报告首条扫查成功，实际运行版本见结果 `metadata.json`。
